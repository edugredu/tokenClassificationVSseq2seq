#!/bin/bash
# Train continual checkpoints with the same hyperparameters as the old seeded scripts.
# Uses codeInstruction.py and optional fallback chat template for base/continual models.

#SBATCH --job-name=ins_cont
#SBATCH --output=/path/Generative/modelsSeeded/logs_continual/%x-%A_%a.out
#SBATCH --error=/path/Generative/modelsSeeded/logs_continual/%x-%A_%a.err
#SBATCH --partition=XX
#SBATCH --gres=gpu:2
#SBATCH --cpus-per-task=32
#SBATCH --mem=140G
#SBATCH --time=30:00:00
#SBATCH --array=0-2

set -euo pipefail
trap 'echo "Error on line $LINENO. Exiting." >&2' ERR

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BASE_DIR="${SLURM_SUBMIT_DIR:-${SCRIPT_DIR}}"
if [ -f "${BASE_DIR}/Instruction/codeInstruction.py" ]; then
  :
elif [ -f "${BASE_DIR}/Generative/Instruction/codeInstruction.py" ]; then
  BASE_DIR="${BASE_DIR}/Generative"
else
  BASE_DIR="${SCRIPT_DIR}"
fi
ROOT_DIR="$(cd "${BASE_DIR}/.." && pwd)"

echo "Iniciando trabajo en $(hostname) a las $(date)"

# Load conda
set +u
source ~/anaconda3/etc/profile.d/conda.sh
export MKL_INTERFACE_LAYER="${MKL_INTERFACE_LAYER:-}"
conda activate /conda_path
set -u

nvidia-smi

export HF_HOME="/path/hf_cache"
export HF_HUB_CACHE="${HF_HOME}/hub"
export TRANSFORMERS_CACHE="${HF_HUB_CACHE}"
mkdir -p "${HF_HUB_CACHE}"

export CUDA_HOME="$CONDA_PREFIX"
export PYTHONNOUSERSITE=1
export PATH="${CONDA_PREFIX}/bin:${PATH}"

export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True,max_split_size_mb:128
export CUDA_LAUNCH_BLOCKING=1
export TOKENIZERS_PARALLELISM=false

huggingface-cli login --token XXX
python -m wandb login XXX

# Resolve seed from array
SEED_LIST=${SEED_LIST:-"42 1234 2025"}
IFS=' ' read -r -a SEEDS <<< "${SEED_LIST}"
SEED_OVERRIDE=${SEED_OVERRIDE:-${SEEDS[$SLURM_ARRAY_TASK_ID]}}
if [ -z "${SEED_OVERRIDE}" ]; then
  echo "Seed not resolved (SEED_LIST=${SEED_LIST}, SLURM_ARRAY_TASK_ID=${SLURM_ARRAY_TASK_ID})"
  exit 1
fi

MODEL_ROOT=${MODEL_ROOT:-"${BASE_DIR}/modelsSeeded/continual"}
LOG_DIR="${BASE_DIR}/modelsSeeded/logs_continual"
mkdir -p "${MODEL_ROOT}" "${LOG_DIR}"

python -c "import torch; torch.cuda.empty_cache(); print('Cache cleared')"

MODELS_DEFAULT=(
  "cont_llama2_cima_e1|${ROOT_DIR}/Continual/llama2/modelCima_llama2/iter1|llama2"
  "cont_llama2_cima_e2|${ROOT_DIR}/Continual/llama2/modelCima_llama2/iter2|llama2"
  "cont_llama2_fda_e1|${ROOT_DIR}/Continual/llama2/modelFDA_llama2/iter1|llama2"
  "cont_llama2_fda_e2|${ROOT_DIR}/Continual/llama2/modelFDA_llama2/iter2|llama2"
  "cont_llama2_cimafda_e1|${ROOT_DIR}/Continual/llama2/modelCimaFDA_llama2/iter1|llama2"
  "cont_llama2_cimafda_e2|${ROOT_DIR}/Continual/llama2/modelCimaFDA_llama2/iter2|llama2"
  "cont_llama3_cima_e1|${ROOT_DIR}/Continual/llama3/modelCima_llama3/model5_hf|llama3"
  "cont_llama3_cima_e2|${ROOT_DIR}/Continual/llama3/modelCima_llama3/model6_hf|llama3"
  "cont_llama3_fda_e1|${ROOT_DIR}/Continual/llama3/modelFDA_llama3/modelFDA_iter1|llama3"
  "cont_llama3_fda_e2|${ROOT_DIR}/Continual/llama3/modelFDA_llama3/modelFDA_iter2|llama3"
  "cont_llama3_cimafda_e1|${ROOT_DIR}/Continual/llama3/modelCimaFDA_llama_3/iter1|llama3"
  "cont_llama3_cimafda_e2|${ROOT_DIR}/Continual/llama3/modelCimaFDA_llama_3/iter2|llama3"
  "cont_llama3_1_cima_e1|${ROOT_DIR}/Continual/llama3_1/modelCima_llama_3_1/model_iter1|llama3_1"
  "cont_llama3_1_cima_e2|${ROOT_DIR}/Continual/llama3_1/modelCima_llama_3_1/model_iter2|llama3_1"
  "cont_llama3_1_fda_e1|${ROOT_DIR}/Continual/llama3_1/modelFDA_llama_3_1/iter1|llama3_1"
  "cont_llama3_1_fda_e2|${ROOT_DIR}/Continual/llama3_1/modelFDA_llama_3_1/iter2|llama3_1"
  "cont_llama3_1_cimafda_e1|${ROOT_DIR}/Continual/llama3_1/modelCimaFDA_llama_3_1/iter1|llama3_1"
  "cont_llama3_1_cimafda_e2|${ROOT_DIR}/Continual/llama3_1/modelCimaFDA_llama_3_1/iter2|llama3_1"
  "cont_llama3_2_cima_e1|${ROOT_DIR}/Continual/llama3_2/modelCima_llama_3_2/model_iter1|llama3_2"
  "cont_llama3_2_cima_e2|${ROOT_DIR}/Continual/llama3_2/modelCima_llama_3_2/model_iter2|llama3_2"
  "cont_llama3_2_fda_e1|${ROOT_DIR}/Continual/llama3_2/modelFDA_llama_3_2/model_iter1|llama3_2"
  "cont_llama3_2_fda_e2|${ROOT_DIR}/Continual/llama3_2/modelFDA_llama_3_2/model_iter2|llama3_2"
  "cont_llama3_2_cimafda_e1|${ROOT_DIR}/Continual/llama3_2/modelCimaFDA_llama_3_2/model_iter1|llama3_2"
  "cont_llama3_2_cimafda_e2|${ROOT_DIR}/Continual/llama3_2/modelCimaFDA_llama_3_2/model_iter2|llama3_2"
)

if [ -n "${MODEL_INDEX:-}" ]; then
  MODELS=( "${MODELS_DEFAULT[$MODEL_INDEX]}" )
else
  MODELS=( "${MODELS_DEFAULT[@]}" )
fi

echo "SLURM_JOB_ID=${SLURM_JOB_ID:-}"
echo "SLURM_ARRAY_TASK_ID=${SLURM_ARRAY_TASK_ID:-}"
echo "MODEL_INDEX=${MODEL_INDEX:-all}"
echo "SEED_OVERRIDE=${SEED_OVERRIDE}"
echo "Total models selected: ${#MODELS[@]}"

model_idx=0

for entry in "${MODELS[@]}"; do
  IFS="|" read -r label model_path family <<< "${entry}"
  PORT_SEED=$((SLURM_JOB_ID + SLURM_ARRAY_TASK_ID + SEED_OVERRIDE + model_idx))
  MASTER_PORT=${MASTER_PORT:-$((10000 + (PORT_SEED % 50000)))}

  case "${family}" in
    llama2)
      NPROC=2
      MAX_SEQ_LEN=2156
      LR=8e-5
      EPOCHS=30
      GRAD_ACC=8
      EVAL_STEPS=100
      EVAL_MAX_NEW_TOKENS=1587
      SAVE_STEPS=100
      SAVE_TOTAL_LIMIT=1
      GRADIENT_CKPT="--gradient_checkpointing"
      ;;
    llama3_2)
      NPROC=2
      MAX_SEQ_LEN=2285
      LR=8e-5
      EPOCHS=30
      GRAD_ACC=8
      EVAL_STEPS=100
      EVAL_MAX_NEW_TOKENS=1793
      SAVE_STEPS=100
      SAVE_TOTAL_LIMIT=1
      GRADIENT_CKPT="--gradient_checkpointing"
      ;;
    llama3_1)
      NPROC=2
      MAX_SEQ_LEN=2285
      LR=8e-5
      EPOCHS=30
      GRAD_ACC=8
      EVAL_STEPS=100
      EVAL_MAX_NEW_TOKENS=1793
      SAVE_STEPS=100
      SAVE_TOTAL_LIMIT=1
      GRADIENT_CKPT="--gradient_checkpointing"
      ;;
    llama3)
      NPROC=2
      MAX_SEQ_LEN=2285
      LR=5e-5
      EPOCHS=30
      GRAD_ACC=8
      EVAL_STEPS=100
      EVAL_MAX_NEW_TOKENS=1793
      SAVE_STEPS=100
      SAVE_TOTAL_LIMIT=1
      GRADIENT_CKPT="--gradient_checkpointing"
      ;;
    *)
      echo "Unknown family: ${family}"
      exit 1
      ;;
  esac

  OUT_DIR="${MODEL_ROOT}/${label}_s${SEED_OVERRIDE}"
  LOG_FILE="${LOG_DIR}/log_optimal_training_${label}_s${SEED_OVERRIDE}.txt"
  INFER_LOG_FILE="${LOG_DIR}/inference_${label}_s${SEED_OVERRIDE}.txt"

  INFER_OUT_DIR="${MODEL_ROOT}/outputPrompts"
  OUT_JSON="${INFER_OUT_DIR}/final_predictions_${label}_s${SEED_OVERRIDE}.jsonl"
  mkdir -p "${INFER_OUT_DIR}"

  SAVE_LIMIT_ARGS=()
  if [ -n "${SAVE_TOTAL_LIMIT}" ]; then
    SAVE_LIMIT_ARGS=( --save_total_limit "${SAVE_TOTAL_LIMIT}" )
  fi

  {
    echo ""
    echo "=== TRAIN START $(date) ==="
    echo "label=${label}"
    echo "family=${family}"
    echo "model_path=${model_path}"
    echo "seed=${SEED_OVERRIDE}"
    echo "nproc=${NPROC}"
    echo "output_dir=${OUT_DIR}"
  } | tee -a "${LOG_FILE}"

  if torchrun --nproc_per_node="${NPROC}" --master_port="${MASTER_PORT}" "${BASE_DIR}/Instruction/codeInstruction.py" \
    --model_path "${model_path}" \
    --train_path "${BASE_DIR}/Instruction/instructed_prompts_train_p0.jsonl" \
    --valid_path "${BASE_DIR}/Instruction/instructed_prompts_valid_p0.jsonl" \
    --template_path "${BASE_DIR}/Instruction/basePromptInstruction.txt" \
    --output_dir "${OUT_DIR}" \
    --per_device_train_batch_size 1 \
    --gradient_accumulation_steps "${GRAD_ACC}" \
    --max_seq_len "${MAX_SEQ_LEN}" \
    --learning_rate "${LR}" \
    --num_train_epochs "${EPOCHS}" \
    --dump_prompts "${LOG_DIR}/prompt_debug_${label}_s${SEED_OVERRIDE}.txt" \
    --use_lora --lora_r 64 --lora_alpha 128 --lora_dropout 0.15 \
    --bf16 \
    --evaluation_strategy steps \
    --eval_steps "${EVAL_STEPS}" \
    --max_eval_samples 50 \
    --eval_max_new_tokens "${EVAL_MAX_NEW_TOKENS}" \
    --eval_temperature 0.1 \
    --save_strategy steps \
    --save_steps "${SAVE_STEPS}" \
    --load_best_model_at_end \
    --metric_for_best_model entity_f1_exact \
    --greater_is_better \
    --warmup_ratio 0.2 \
    --seed "${SEED_OVERRIDE}" \
    --use_fallback_chat_template \
    ${GRADIENT_CKPT} \
    "${SAVE_LIMIT_ARGS[@]}" \
    >> "${LOG_FILE}" 2>&1; then
    echo "=== TRAIN DONE $(date) ===" | tee -a "${LOG_FILE}"
  else
    echo "=== TRAIN FAILED $(date) ===" | tee -a "${LOG_FILE}"
    exit 1
  fi

  # Final inference + official evaluation on the test set.
  MAX_NEW_TOKENS=860
  TEMPERATURE=0.17315939349886061
  REPETITION_PENALTY=1.0069760044869343
  NUM_BEAMS=3
  TOP_P=0.7005801448613161
  DO_SAMPLE_FLAG=
  NO_REPEAT_NGRAM_SIZE=6

  case "${family}" in
    llama2)
      MAX_NEW_TOKENS=1326
      TEMPERATURE=0.3327288653275695
      REPETITION_PENALTY=1.5541629428392234
      NUM_BEAMS=4
      TOP_P=0.8021746621540992
      ;;
    llama3_2)
      MAX_NEW_TOKENS=1124
      TEMPERATURE=0.24899144287235947
      REPETITION_PENALTY=1.327448913609007
      NUM_BEAMS=3
      TOP_P=0.8253425573316452
      ;;
    llama3_1)
      MAX_NEW_TOKENS=1071
      TEMPERATURE=0.0006183203210246985
      REPETITION_PENALTY=1.957728614827438
      NUM_BEAMS=3
      TOP_P=0.70614848048016
      DO_SAMPLE_FLAG=--do_sample
      ;;
    llama3)
      MAX_NEW_TOKENS=1266
      TEMPERATURE=0.06011203590107267
      REPETITION_PENALTY=1.398796712930793
      NUM_BEAMS=8
      TOP_P=0.9130844873923798
      DO_SAMPLE_FLAG=--do_sample
      ;;
  esac

  {
    echo ""
    echo "=== INFER START $(date) ==="
    echo "label=${label}"
    echo "family=${family}"
    echo "model_path=${model_path}"
    echo "seed=${SEED_OVERRIDE}"
    echo "lora_dir=${OUT_DIR}"
    echo "output_json=${OUT_JSON}"
  } | tee -a "${INFER_LOG_FILE}"

  if python "${BASE_DIR}/Instruction/infer_entities.py" \
    --base_model_path "${model_path}" \
    --lora_dir "${OUT_DIR}" \
    --output_path "${OUT_JSON}" \
    --max_new_tokens "${MAX_NEW_TOKENS}" \
    --temperature "${TEMPERATURE}" \
    --num_beams "${NUM_BEAMS}" \
    --top_p "${TOP_P}" \
    --repetition_penalty "${REPETITION_PENALTY}" \
    --no_repeat_ngram_size "${NO_REPEAT_NGRAM_SIZE}" \
    ${DO_SAMPLE_FLAG} \
    --max_samples 0 \
    --search_method none \
    >> "${INFER_LOG_FILE}" 2>&1; then
    echo "=== INFER DONE $(date) ===" | tee -a "${INFER_LOG_FILE}"
  else
    echo "=== INFER FAILED $(date) ===" | tee -a "${INFER_LOG_FILE}"
    exit 1
  fi

  BASE_PREFIX="${OUT_JSON%.*}"
  GOLD_DIR="${BASE_PREFIX}_brat_gold"
  PRED_RAW_DIR="${BASE_PREFIX}_brat_raw"
  PRED_FIXED_DIR="${BASE_PREFIX}_brat_fixed"
  {
    echo ""
    echo "Running official PharmacoNER evaluation (raw)..."
    python "${BASE_DIR}/../PharmacoNER/Evaluacion/evaluate.py" ner "${GOLD_DIR}" "${PRED_RAW_DIR}"
    echo ""
    echo "Running official PharmacoNER evaluation (fixed)..."
    python "${BASE_DIR}/../PharmacoNER/Evaluacion/evaluate.py" ner "${GOLD_DIR}" "${PRED_FIXED_DIR}"
  } >> "${INFER_LOG_FILE}" 2>&1

  model_idx=$((model_idx + 1))
done

echo "Trabajo completado a las $(date)"