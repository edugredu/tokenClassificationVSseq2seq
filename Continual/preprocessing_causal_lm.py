...
from datasets import DatasetDict, Dataset, Features, Value, load_from_disk, Sequence
...
def create_dataset_from_generator(data_files: str, dataset_name: str):
    ...
    dataset_dict = DatasetDict({
        ...
        'validation': split_dataset['test']  # Renaming 'test' split to 'validation'
    })
...
def load_dataset_test_memory(dataset_name: str):
    ...
    print(f"Loaded dataset '{dataset_name}' with {len(dataset['train'])} train examples and {len(dataset['validation'])} valid examples")
...
def load_dataset_from_disk(path = None):
    if path is None:
        print("ALERT: Path is None")
        path = "/usrvol/valencian_dataset"
...
if __name__ == "__main__":
    ...
    split_files_in_directory(directory=directory_to_scan, output_dir=output_dir, max_size=max_file_size)
